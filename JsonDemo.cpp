...
#else
      _snprintf(buffer, sizeof(buffer), "[%u]", index);
#endif
...
int zdTestParseJson()
{
	JSONCPP_STRING strJsonIn = "";
	JSONCPP_STRING strActualPath="d:\\zdtest.json";
	JSONCPP_STRING strKind="input";
	Json::Features strFeatures;
	...
	return parseAndSaveValueTree(strJsonIn, strActualPath, strKind, strFeatures, bParseOnly, &jsonRoot);
}
